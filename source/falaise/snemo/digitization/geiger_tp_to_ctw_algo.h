...
namespace snemo {
  ...
  namespace digitization {
    ...
    public :
      ...
      void process(const geiger_tp_data & tp_data_,
									 geiger_ctw_data & ctw_data_);

    protected :

      /// Fill a geiger ctw from a geiger tp
      void _fill_a_geiger_ctw(const geiger_tp & my_geiger_tp_,
															geiger_ctw & a_geiger_ctw_);

      /// Process to fill a ctw from a list of geiger tp for a clocktick
      void _process_for_a_ctw_for_a_clocktick(const std::vector<datatools::handle<geiger_tp> > & my_list_of_geiger_tp_,
																							geiger_ctw & a_geiger_ctw_);
