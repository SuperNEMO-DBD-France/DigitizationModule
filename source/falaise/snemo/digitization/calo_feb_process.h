...
namespace snemo {
  ...
  namespace digitization {
	...
	static const unsigned int DEFAULT_ZERO_ADC_POS = DEFAULT_ADC_DYNAMIC / 2;

	static const unsigned int ADC_DYNAMIC = DEFAULT_ADC_DYNAMIC; //!< ADC dynamic
	static const unsigned int NUMBER_OF_SAMPLES_FOR_BASELINE = 16;

	double DEFAULT_VOLTAGE_DYNAMIC;
	double VOLT_ADC_VALUE;
	...
	double  sampling_rate;	 //!< Wavecatcher sampling rate (def : 2,56 GHz
	double  sampling_step;	 //!< Wavecatcher sampling step (def : 390,625 ps)
	double  timestamping_rate; //!< Wavecatcher timestamping rate (def : 160 MHz)
	double  timestamping_step; //!< Wavecatcher timestamping step (def : 6,25 ns)
	double  post_trig_window_ns; //!< Post trigger : Time to record after trigger
	...
	void reset();
	void calculate_metadata(const calo_feb_config * feb_config_,
				snemo::datamodel::sim_calo_digi_hit & SCDH_);
	void readout(const calo_feb_config * feb_config_,
		     snemo::datamodel::sim_calo_digi_hit & SCDH_);
