  ...
  namespace digitization {
    ...
    public :
      ...
      void reset();
			...
			void set_ctw_htm(const calo_tp & my_calo_tp_,
											 calo_ctw & my_ctw_);

			/// Set the ctw zone bit for a given clocktick when the high threshold is passed
      void set_ctw_zone_bit_htm(const calo_tp & my_calo_tp_,
																calo_ctw & my_ctw_);

			/// Set the ctw low threshold only for a given clocktick
			void set_ctw_lto(const calo_tp & my_calo_tp_,
											 calo_ctw & my_ctw_);

			/// General proces to fill a ctw data object from a list of calo tp
			void process(const calo_tp_data & tp_data_,
									 calo_ctw_data & ctw_data_);

		protected :

			/// Fill a main wall calo ctw from a main calo tp
			void _fill_a_main_wall_ctw(const calo_tp & my_calo_tp_,
																 calo_ctw & a_calo_ctw_);

			/// Fill a xwall gveto calo ctw from a xwall or gveto calo tp
			void _fill_a_xwall_gveto_ctw(const calo_tp & my_calo_tp_,
																	 calo_ctw & a_calo_ctw_);

			/// Process for a ctw for a clocktick for main wall
			void _process_for_a_ctw_for_a_clocktick_for_main_wall(const std::vector<datatools::handle<calo_tp> > & my_list_of_calo_tp_,
																														calo_ctw & a_calo_ctw_);

			/// Process for a ctw for a clocktick for xwall gveto wall
			void _process_for_a_ctw_for_a_clocktick_for_xwall_gveto(const std::vector<datatools::handle<calo_tp> > & my_list_of_calo_tp_,
																															calo_ctw & a_calo_ctw_);
